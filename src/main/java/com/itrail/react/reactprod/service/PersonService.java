...
public class PersonService {
    ...
    public Mono<Person> findByIdPerson( Long id ) throws Exception{
        return  repository.findById( id ).switchIfEmpty( Mono.error( new NoSuchElementException("Нет персона с таким ИД")));
    }

    public Mono<Person> updatePerson( Person person ) throws Exception{
        repository.findById( person.getId() ).switchIfEmpty( Mono.error( new NoSuchElementException( "Нет персона с таким ИД")));
